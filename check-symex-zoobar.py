...
def test_stuff():
  ...
  user1 = sum([1 for p in pdb.query(zoobar.zoodb.Person).all()])
  totalBalances1 = {p.username:p.zoobars for p in pdb.query(zoobar.zoodb.Person).all()}
  pdb.commit()
  ...
  userEnd = sum([1 for u in pdb.query(zoobar.zoodb.Person).all()])
  if balanceEnd != balance1 and userEnd == user1:
    report_balance_mismatch()

  ## Detect zoobar theft.
  ## When detected, call report_zoobar_theft()
  totalBalancesEnd = {p.username:p.zoobars for p in pdb.query(zoobar.zoodb.Person).all()}
  if len(totalBalancesEnd.keys()) == len(totalBalances1.keys()) and set(totalBalancesEnd.keys()) == set(totalBalances1.keys()):
    diffBalanceUsers = []
    for user in totalBalances1:
      if totalBalances1[user] != totalBalancesEnd[user]:
        diffBalanceUsers.append(user)

  tdb = zoobar.zoodb.transfer_setup()
  for user in diffBalanceUsers:
    changeBalance = 0
    userTransfer = tdb.query(zoobar.zoodb.Transfer).filter_by(sender=user)
    for transfer in userTransfer:
      changeBalance -= transfer.amount

    userTransfer = tdb.query(zoobar.zoodb.Transfer).filter_by(recipient=user)
    for transfer in userTransfer:
      changeBalance += transfer.amount

    if totalBalancesEnd[user] != totalBalances1[user] + changeBalance:
      report_zoobar_theft()
